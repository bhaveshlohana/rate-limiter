...
public class RateLimitStatusService {
    ...
    public RateLimitStatus getRateLimitStatus(RateLimitRequest rateLimitRequest) {
        ...
        return switch (config.getAlgorithm()) {
            case FIXED_WINDOW -> getFixedWindowStatus(rateLimitRequest, config);
            case SLIDING_WINDOW -> getSlidingWindowStatus(rateLimitRequest, config);
            case TOKEN_BUCKET -> getTokenBucketStatus(rateLimitRequest, config);
