package com.bhavesh.learn.rate_limiter.service;

import com.bhavesh.learn.rate_limiter.core.FixedWindowRateLimiter;
import com.bhavesh.learn.rate_limiter.core.SlidingWindowRateLimiter;
import com.bhavesh.learn.rate_limiter.core.TokenBucketRateLimiter;
import com.bhavesh.learn.rate_limiter.exception.ConfigNotFoundException;
import com.bhavesh.learn.rate_limiter.model.RateLimitConfig;
import com.bhavesh.learn.rate_limiter.model.RateLimitRequest;
...
public class RateLimiterFactory {
    ...
    public RateLimitResponse applyRateLimiting(RateLimitRequest rateLimitRequest) {
        ...
        RateLimitResponse response = switch (config.getAlgorithm()) {
            case FIXED_WINDOW -> fixedWindowRateLimiter.isAllowedLua(userId, config);
            case SLIDING_WINDOW -> slidingWindowRateLimiter.isAllowedLua(userId, config);
            case TOKEN_BUCKET -> tokenBucketRateLimiter.isAllowedLua(userId, config);
        };
